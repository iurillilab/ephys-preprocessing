...
from spikeinterface import aggregate_channels, create_sorting_analyzer
...
def get_stream_name(
    folder: Path, data_type: Literal["events", "continuous"] = "continuous"
) -> str:
    """Get stream name from folder."""
    folder = Path(folder)
    STREAM_NAME_MATCH = (
        f"Record Node */experiment*/recording*/{data_type}/Neuropix-PXI-*.Probe*"
    )
    try:
    ...
    for item in tqdm(list(src.rglob("*")), desc="Copying files", unit="file"):
        if item.is_file():
            ...
            if not dest_path.exists():
                ...
                if (
                    item.stat().st_mtime != dest_path.stat().st_mtime
                    or item.stat().st_size != dest_path.stat().st_size
                ):
                    should_copy = True
...
class RecordingProcessor:
    ...
    def __post_init__(self):
        ...
        self.root_working_dir = (
            Path(self.root_working_dir) if self.root_working_dir is not None else None
        )

        # Initialize all paths
        self.processed_in_different_folder = self.root_working_dir is not None
        self.local_folder = (
            self.root_working_dir / self.source_path.name
    ...
    def try_load_recording(self) -> OpenEphysBinaryRecordingExtractor | None:
        ...
        try:
            if self.is_split_recording:
                folders = sorted(
                    f for f in self.local_folder.glob("*") if is_timestamp_folder(f)
                )
                if not folders:
                    return None
                ...
                return OpenEphysBinaryRecordingExtractor(
                    self.local_folder, stream_name=self.stream_name
                )
        except Exception as e:
            print(
                f"Failed to load recording in {self.local_folder} (Split: {self.is_split_recording} ): {e}"
            )
            return None
    ...
    def try_load_sorter(self) -> si.BaseSorting | None:
        ...
        try:
            return si.read_kilosort(
                self.kilosort_folder / "sorter_output", keep_good_only=False
            )
        except Exception as e:
    ...
    def try_load_analyzer(self) -> si.SortingAnalyzer | None:
        ...
        try:
            return si.load_sorting_analyzer(
                self.analyzer_folder, format="binary_folder"
            )
        except Exception as e:
    ...
    def copy_results_to_source_dir(self, overwrite: bool = True) -> None:
        ...
        if self.processed_in_different_folder:
            ...
            print(
                f"Would copy results to source directory, but we are in the same folder: {self.local_folder}"
            )
    ...
    def cleanup_working_dir(self) -> None:
        ...
        if self.processed_in_different_folder:
            ...
            print(
                f"Would cleanup working directory, but we are in the same folder: {self.local_folder}"
            )
...
def standard_preprocessing(
    ...
    recording = spre.interpolate_bad_channels(
        recording, bad_channel_ids=bad_channel_ids
    )

    if recording.get_channel_groups().max() > 1:
        grouped_recordings = recording.split_by(property="group")
        recgrouplist_hpsf = [
            spre.highpass_spatial_filter(recording=grouped_recordings[k])
...
def run_sorting(
    recording: OpenEphysBinaryRecordingExtractor, kilosort_folder: Path, overwrite: bool
) -> si.BaseSorting:
...
def compute_stats(
    sorting_ks: si.BaseSorting,
    recording: OpenEphysBinaryRecordingExtractor,
    sortinganalyzerfolder: Path,
) -> None:
    ...
    analyzer.compute(
        "random_spikes", method="uniform", max_spikes_per_unit=500, **job_kwargs
    )
...
def process_recording(
    ...
    if recording is None:
        if dry_run:
            ...
            print(
                f"Copying took {_n_seconds_to_formatted_time(time.time()-copy_t_start)}"
            )
    ...
    if sorting is None or overwrite:
        if dry_run:
            ...
            sorting = processor.try_load_sorter()
            print(
                f"Sorting duration: {_n_seconds_to_formatted_time(time.time()-sorting_t_start)}"
            )
    else:
    ...
    if analyzer is None or overwrite:
        if dry_run:
            ...
            print(
                f"Stats duration: {_n_seconds_to_formatted_time(time.time()-stats_t_start)}"
            )
...
def find_recordings_to_process(
    source_dir: Path, root_working_dir: Path
) -> list[RecordingProcessor]:
...
luigi_macbook_kwargs = RunArgs(
    source_dir=Path(
        "/Users/vigji/Desktop/short_recording_oneshank"
    ),  # Path("/Volumes/Extreme SSD/mouse_data_electrode_tips"),
...
wsl_run_kwargs = RunArgs(
    source_dir=Path(
        "/mnt/nas/SNeuroBiology_shared/P07_PREY_HUNTING_YE/e01_ephys _recordings"
    ),
...
def main():
    ...
    processors = find_recordings_to_process(
        run_kwargs.source_dir, run_kwargs.root_working_dir
    )
    pprint(processors)
    ...
    for processor in processors:
        ...
        process_recording(
            ...
            overwrite=run_kwargs.overwrite,
            copy_upstream=run_kwargs.copy_upstream,
            cleanup=run_kwargs.cleanup,
        )
