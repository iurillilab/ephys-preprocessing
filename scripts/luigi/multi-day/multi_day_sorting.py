...
si.set_global_job_kwargs(
    n_jobs=os.cpu_count() // 2,  # physical cores hald of virtual total
    progress_bar=True,
)  # chunk_duration="1s"
...
def _convert_windows_path_to_linux(path_str: str) -> Path:
    SPECIAL_DRIVE_MAP = {
        "N": Path("/mnt/nas"),
        "D": Path("/mnt/d"),
    }  # drives that are not just lowercase letter
    # mount just in case the nas:
    if not SPECIAL_DRIVE_MAP["N"].exists():
        subprocess.run(
            "sudo mount -t drvfs \\\\10.231.128.151\\SystemsNeuroBiology /mnt/nas"
        )
    if os.name != "posix" or str(path_str).startswith("/mnt/"):
        return Path(path_str)
    p = PureWindowsPath(path_str)
    drive_letter = p.drive.rstrip(":").upper()
    return SPECIAL_DRIVE_MAP.get(
        drive_letter, Path(f"/mnt/{drive_letter.lower()}")
    ).joinpath(*p.parts[1:])
...
test_rec_tuple = (
    Path(_convert_windows_path_to_linux(test_rec_tuple[0])),
    test_rec_tuple[1],
)
main_working_dir = _convert_windows_path_to_linux(main_working_dir)
assert (
    main_working_dir.exists()
), f"Main working directory {main_working_dir} does not exist."
assert source_dir.exists(), f"Source directory {source_dir} does not exist."
if not test_rec_tuple[0].exists():
    print(
        f"Test recording {test_rec_tuple[0]} does not exist, won't be able to run test mode."
    )
...
def get_stream_name(folder):
    ...
    STREAM_NAME_MATCH = (
        "Record Node */experiment*/recording*/continuous/Neuropix-PXI-*.Probe*"
    )
    # stream_path =
...
for mouse_id, mouse_path in list(mouse_paths.items())[:1]:
    ...
    timestamp_folders = sorted(
        folder
        for folder in mouse_path.rglob("*-*_*/")
    ...
    for timestamp_folder in timestamp_folders:
        try:
            ...
            recording_raw = OpenEphysBinaryRecordingExtractor(
                timestamp_folder, stream_name=stream_name
            )
        except Exception as e:
            print(
                f"Error processing {timestamp_folder}: {repr(e)} ({type(e)}). Is this a valid OpenEphys folder?"
            )
            traceback.print_exc()
            continue
        ...
        if "split_" in timestamp_folder.parent.name:
            ...
            pooled_dirs_to_process[timestamp_folder.name] = [
                timestamp_folder,
                stream_name,
            ]
...
def standard_preprocessing(recording_extractor):
    ...
    if recording.get_channel_groups().max() > 1:
        ...
        ]  # cmr is slightly faster. results are similar
...
def compute_stats(
    sorting_data_folder, sorter_object, recording, logger=None
):  # , **job_kwargs):
    ...
    for file_path in found_paths:
        ...
        if sparsity is None:
            ...
            sparse_channels_indices = {
                unit_id: np.arange(num_channels)
...
def process_recording(processor: RecordingProcessor, overwrite: bool = False) -> None:
    ...
    global_t_start = time.time()
    print(
        f"Processing {processor.source_path.name} with stream name {processor.stream_name}"
    )
