...
#####
nas_data_path = Path(
    "/Volumes/SystemsNeuroBiology/SNeuroBiology_shared/P07_PREY_HUNTING_YE/e01_ephys _recordings/M30"
)
...
if local_csv_filename.exists():
    ...
    ks_paths = list(nas_data_path.glob("*/kilosort4")) + list(
        nas_data_path.glob("*/*/kilosort4")
    )
    print(len(ks_paths))
    all_dfs = []
    for ks_path in tqdm(ks_paths):
        ...
        mouse_id = (
            ks_path.parent.parent.parent.name
            if "split" in ks_path.parent.parent.name
            else ks_path.parent.parent.name
        )
...
all_dfs["allen_filter"] = (
    (all_dfs.isi_violations_ratio < 0.5)
    & (all_dfs.amplitude_cutoff < 0.1)
    & (all_dfs.presence_ratio > 0.9)
)
all_dfs["ibl_filter"] = (all_dfs.sliding_rp_violation < 0.1) & (
    np.abs(all_dfs.amplitude_median) > 50
)
...
print(
    f"Allen & IBL criteria: {(all_dfs['allen_filter'] & all_dfs['ibl_filter']).sum()}"
)
print(f"KS and IBL criteria: {(all_dfs['ibl_filter'] & all_dfs['KS_filter']).sum()}")
print(
    f"KS and Allen criteria: {(all_dfs['allen_filter'] & all_dfs['KS_filter']).sum()}"
)
print(
    f"All three criteria: {(all_dfs['allen_filter'] & all_dfs['ibl_filter'] & all_dfs['KS_filter']).sum()}"
)
...
plt.title(
    "Unit Quality Criteria Overlap\n(All sets are subsets of total units)",
    pad=20,
    size=14,
)
plt.show()
...
mouse_df.groupby("recording_date").agg(
    {"KS_filter": "sum", "allen_filter": "sum", "ibl_filter": "sum"}
)
daily_counts = (
    mouse_df.groupby("recording_date")[["KS_filter", "allen_filter", "ibl_filter"]]
    .sum()
    .reset_index()
)
