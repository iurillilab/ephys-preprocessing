...
for mouse_id, mouse_path in mouse_paths.items():
    ...
    unique_timestamps = sorted(
        list(set(folder.name.split("_")[0] for folder in timestamp_folders))
    )
    ...
    for timestamp in unique_timestamps:
        ...
        first_matching_folder = next(
            folder for folder in timestamp_folders if folder.name.startswith(timestamp)
        )
        first_matching_folder

        stream_name = "Record Node 107#Neuropix-PXI-100.ProbeA"
        recording_raw = OpenEphysBinaryRecordingExtractor(
            first_matching_folder, stream_name=stream_name
        )
        ...
        window_starts = np.linspace(
            0, filtered.get_num_frames() - num_samples, num_windows, dtype=int
        )
        channel_ids = filtered.channel_ids
        # print(window_starts)
        rms_map = np.zeros((len(channel_ids), num_windows))
        for i in trange(num_windows):
            traces = filtered.get_traces(
                start_frame=i * num_samples, end_frame=(i + 1) * num_samples
            )
...
for (mouse_id, all_rms_list), lims in zip(
    all_rms_list_dict.items(), [(68, 72), (66, 72), (66, 72)]
):
    ...
    for i, rms_map in enumerate(all_rms_list):
        axs[i].imshow(
            ...
            origin="lower",
            cmap="viridis",
            extent=[0, filtered.get_num_frames() / fs, 0, len(channel_ids)],
            vmin=lims[0],
            vmax=lims[1],
        )
    ...
    plt.show()
    f.savefig(
        Path(r"D:\SNeurobiology\Desktop\multiday_ephys") / f"rms_{mouse_id}.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close(f)
